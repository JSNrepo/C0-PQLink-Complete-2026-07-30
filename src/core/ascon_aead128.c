...
)
{
    ...
    if (!c0pqlink_ct_equal(expected_tag, tag, sizeof(expected_tag))) {
        c0pqlink_secure_zero(plaintext_start, original_length);
        c0pqlink_secure_zero(expected_tag, sizeof(expected_tag));
        c0pqlink_secure_zero(&state, sizeof(state));
